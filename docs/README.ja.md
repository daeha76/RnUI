...
- **56 のコンポーネントカテゴリ** — 195 以上の Razor コンポーネントファイルを含む包括的な UI ライブラリ
...
| `RnDateField` | RnField + RnFieldLabel + RnDatePicker の便利なラッパー（string? バインディング） | [Live](https://daeha76.github.io/RnUI/components/date-field) |
...
RnField + RnFieldLabel + RnDatePicker を1つのコンポーネントにまとめた便利なラッパーです。簡単な移行のために `string?` バインディング（yyyy-MM-dd 形式）を受け付けます。

```razor
<RnDateField Label="生年月日" @bind-Value="_birthDate" />
<RnDateField Label="開始日" @bind-Value="_startDate" Placeholder="日付を選択..." />
```
