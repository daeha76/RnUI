...
> **이 커맨드는 `/sync-block`에서 L1 컴포넌트 차이가 발견되었을 때 먼저 실행해야 합니다.**
> 참조: `.agents/knowledge/sync-strategy.md`
...
5. **파급 효과 확인**: 이 컴포넌트를 사용하는 블록/데모 목록을 검색하여 영향 범위를 알려줍니다.

6. **변환 가이드**: 차이가 있다면 Blazor로 어떻게 변환해야 하는지 구체적 코드를 제안합니다.
...
- 컴포넌트 수정 시 파급 효과(다른 블록/데모)를 확인하지 않는 것
- CSS 비교 없이 HTML 구조만 비교하는 것
